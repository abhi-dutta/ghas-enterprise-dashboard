...
export default function Sidebar({ filters, set, clear, hasActive, onUpload }: Props) {
  ...
  return (
    <aside style={{
      ...
      <div>
        ...
        <input
          style={inputStyle}
          placeholder="CVE, package, repo, org…"
      ...
      {isLoading ? (
        ...
        <>
          <MultiSelect label="Severity"  options={opts.severities} value={filters.severity}  onChange={v => set('severity', v)} />
          <MultiSelect label="State"     options={opts.states}     value={filters.state}     onChange={v => set('state', v)} />
          <MultiSelect label="Ecosystem" options={opts.ecosystems} value={filters.ecosystem} onChange={v => set('ecosystem', v)} />
...
// ── Org Typeahead ──────────────────────────────────────────────────────────
function OrgTypeahead({ value, onChange }: {
  ...
  return (
    <div ref={ref}>
        ...
        <div style={{
          ...
          {filtered.map(org => (
            <button key={org} onClick={() => { toggle(org); setQuery('') }} style={{
              ...
              padding: '6px 10px', fontSize: 12, color: '#d1d5db',
      ...
      {open && query && filtered.length === 0 && (
        <div style={{
          marginTop: 4, padding: '8px 10px', fontSize: 12, color: '#6b7280',
          background: '#1f2937', border: '1px solid #374151', borderRadius: 6,
        }}>
